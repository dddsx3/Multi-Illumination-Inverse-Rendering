...
> 日期：2026-09-04 · 严重度：🟠（闭合后转记录）· 状态：**CLOSED**（选项 A：scene 级唯一口径，三层验收全绿）
...
## 闭合记录（2026-09-04 · 作者裁决 = 选项 A · scene 级唯一口径）

- **裁决**：选项 A——scene 级（batch=1）为唯一评估口径。
- **落地**：evaluate_model.py 改 batch=1（含注释）；A3-0 test 以 scene 级重跑 → 新产物
  `eval_output/A3-0_f_n5gray_seed42_test_v2_scenelevel/`（旧 batch 池化版 *_test 标注作废，留审计）。
- **三层验收**：① 对齐——scene 级 test normal MAE **14.887°** = n_curve N=5 14.887°，albedo
  si-MAE **0.0543** = 0.0543（n_curve 本就 scene 级，**无需重跑 n_curve**，省 1–2h GPU）✓；
  ② std 窗口条款随选项 A 修订——scene 级 std 12.62 自洽（旧 3.75 系 batch 池化抹平，条款作废）；
  ③ 平坦性复核——N1..N5 14.875/14.870/14.880/14.886/14.887，极差 0.017° ≤0.5°，**N_min=1 保留** ✓。
- **数字刷新**：README §4.1 Gen-A3 行、CLAIM_CARDS S-01（scene 级 14.89°/0.0543）与 S-02（冻结
  Gen-A3 曲线）、v2.1 R-F/R-G（判据 4 ≤0.065）全部更新至 scene 级。
- **遗留**：历史世代数字均为 batch 池化口径（7.792° 等），与 scene 级不可直接比较——reference-only
  用途下加"batch 口径"注记（S-01 卡已注）；如需同口径化属 P1 批次。
- 状态：**CLOSED**（G0 附件可引本闭合记录）。

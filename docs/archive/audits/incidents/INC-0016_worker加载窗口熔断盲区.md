...
**根因修复（22:5x 第二次死锁后定位）**：第一次 kill 后重启仍传 `--num_workers 0`，但
`main.py:804` 的判断是 `if args.num_workers and args.num_workers > 0`——**0 被当作"哨兵=未传"**，
config 默认 4 worker 生效 → spawn 死锁原样再现（worker 16800 同款形态）。这是 INC-0009 的镜像
bug（那次是参数漏传，这次是 0 值语义歧义）。修复（commit 6727ef9）：argparse 哨兵 default=0 →
**None**；显式传任意值（含 0）都覆盖 config；直跑不传保持 config 默认，直跑口径不变。
第三次启动（23:0x，nw=0 真正生效）**epoch 0 正常落盘、epoch 1/2 按 271s 节律推进、GPU 周期计算
80°C——死锁解除实证**。教训：**"显式传 0"与"未传"在任何 CLI 参数语义中都必须可区分**，凡用
0 当哨兵的接口在 0 是合法业务值时必炸（INC-0009 家族第二例）。

**P0 处置（立即生效，§6.1 已含根因修复后状态）**：
...
2b. ~~（已被根因修复取代）~~ main.py nw=0 哨兵语义修复（6727ef9）——前两次"处置 1+2"本应生效，实际被 main.py 的 0=未传 哨兵劫持；

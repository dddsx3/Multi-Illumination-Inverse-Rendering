...
## 追加：cam_dist 2.2 重渲验收（G2 复验，数据集升级为 v2）

- 8 场景试点确认改善（覆盖均值 0.186 -> 0.242）后全量重渲：21 块中 1 块
  exit=1（分块隔离生效），最终 **633 个有效场景**
- 清洗 3 个场景：1 个 PSNR 11.8dB 边缘 + 2 个覆盖 0.958 超上限
- **validate_dataset PASS**：覆盖 mean **0.250**（达标 [0.2,0.8]）、法线夹角
  0.01°、PSNR mean 23.3dB / min 12.1dB
- 数据集位置：`D:/data/synthetic_v2/`；旧版保留于 `D:/data/synthetic/`
- 已基于 v2 启动 BF16 全量训练，完成后更新 T1.6 基线

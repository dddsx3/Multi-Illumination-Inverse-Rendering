...
| R-F | A3-3 判据包 | 判据 4（albedo 保护）基线：~~1.2×0.0532 = 0.064~~ → **1.2×A3-0 albedo（0.1482）= 0.178**（待 EX-01 n_curve 后与判据 1 参照系一并定稿）；判据 1 参照系：~~F-N5 全平 0.03°~~ → **A3-0 世代 n_curve 实测曲线（EX-01 产出后回填）** | ⏳ 待 EX-01 |
| R-G | §5 附录判据数字总表 | 三行更新：albedo 保护 ≤0.064→≤0.178（临时，待 R-F 定稿）；真实组/冻结纪律行不变；新增行：世代指纹（RUN_CARD 三指纹必须齐全才能验收 run） | ⏳ 与 R-F 同批 |

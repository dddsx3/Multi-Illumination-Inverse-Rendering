...
协议（估计器侧 corruption——真实数据的受控注入形态，跑前预注册）：
  ...
  nominal（corruption 前，GT/nominal 计算，禁反调）：
    ...
    4. 预测退化：弱 5 模式（F∞^{-1/2}ΔF 基）deg_pred_j = λ_j(F∞)/λ_j(ΔF) ≥ 1。
  ...
  主检验（Gate D / H5）：deg_pred vs deg_emp 的 Spearman + bootstrap 95% CI
...
def noise_fit(I, s_hat):
    """异方差噪声模型 (a,b)：Var ≈ a + b·I（亮区拟合）。"""
    resid = I - s_hat
    sel = s_hat.ravel() > 0.05
    if sel.sum() < 200:
        return 1e-4, 0.0
    a_, b_ = np.polyfit(s_hat.ravel()[sel], (resid.ravel() ** 2)[sel], 1)
    return (1e-4, 0.0) if a_ < 0 else (float(a_), float(b_))
...
class NominalScene:
    ...
    def delta_f_injected(self, sig_phi_diag):
        ...
        sig = np.diag(np.asarray(sig_phi_diag, float)) if np.ndim(sig_phi_diag) == 1             else np.asarray(sig_phi_diag, float)
    ...
    def predicted_degradation(self, sig_phi_diag):
        sig = np.diag(np.asarray(sig_phi_diag, float)) if np.ndim(sig_phi_diag) == 1             else np.asarray(sig_phi_diag, float)
        ...
        pred_deg = 1.0 / rho_R[weak]                          # λ_j(F∞)/λ_j(ΔF) ≥ 1
        return pred_deg, V[:, weak]
...
def run(config, run_dir):
    ...
    for obj_name in objects:
        ...
        scen = NominalScene(obj, rng)
        V_cache = {}
        for level in levels:
            gen = CorruptionGenerator(ctype, level)
            sig_phi = gen.sigma_phi_diag()                   # (3,3) 物理单位
            pred_deg, Vw = scen.predicted_degradation(np.diag(sig_phi))
            V_cache[level] = Vw
            ...
            for s_i in range(seeds_per_level):
                ...
                I_star = scen.s_hat * scen.rho[None, :] +                     s_rng.choice(resid_pool, size=scen.I.shape, replace=True)
    ...
    # ---- Gate D 主检验：Spearman + 全局仿射尺度 + bootstrap CI ----

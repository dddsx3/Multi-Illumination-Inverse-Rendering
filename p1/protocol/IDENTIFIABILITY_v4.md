...
- **OPEN-6**（OPEN-5 引出的新缺口）：复合类场景"组件间光度再分配"歧义的形式化建模——
  ...
  **一稿已执行（2026-09-04，CPU 轨 4-3）**：`p1/protocol/open6_compound_mechanism_check.py`
  ——双组件守恒再分配 δa（sum=0）在 rank(Y)=9 时被逐光最小二乘 δc **完全补偿**（每光
  残差 0.0000）⇒ 机制 (b) 的 Jacobian 零方向构造数值成立，与 §2.2 反例族（rankMesh=9
  但 near_zero>1 的复合类）自洽。**仍属数值候选 ≠ 证明**，冻结前需 T2-4 外部核对。

---

## 6. T2-4 外部核对排期（2026-09-04 登记，CPU 轨 4-3）

- 渠道：复用 EXPERT_REVIEW_PACKAGE 模式（EXPERT_BRIEFING.md §4 问题清单法）——打包
  v4 草案 + open6 脚本 + W1D3 A-P3 证据表，作为**新增问题 6**提交：
  "复合场景组件间光度再分配零方向的构造（OPEN-6 一稿）在 PS/逆渲染文献中是否有等价
  刻画？（对照 closest_prior_verified.md §5 SCPS-NIR 的分解歧义口径）"。
- 排期：**G0（09-13）后统一打包发送**（与 G0 裁决书的理论证据清单合批，避免两次打扰）；
  若 G0 前需要理论背书，单独走 EXPERT_BRIEFING 更新通道。
- 本文档冻结条件不变：T2-4 外部核对通过 → v4 FROZEN；未通过 → 按核对意见修订后重核。

---

*草案 v0.4 · 2026-09-04（v0.3 基础上：OPEN-6 数值一稿——组件守恒再分配零方向被 δc 完全补偿，机制 (b) 构造闭合；新增 §6 T2-4 外部核对排期）· 未经外部核对，禁止引用为已证结论。下一步：G0 后 T2-4 外部核对合批。*

...
- **解析补光域（协议模型重打光，数据修复后的参考预期，124 test 场景）**：
  SI-MAE(A) 从 N=1 的 0.077 降至 N=5 的 0.020（**3.8×**），
  N≥5 饱和（详细见 `information_audit/INFORMATION_AUDIT.md §1`）。
  证明协议模型与估计器本身能从真多光照数据中提取信息——瓶颈确证
  在数据，不在方法。
...
- 解析域（σ=0.005 噪声，124 场景）：**Δ_new = −0.00118，Δ_dup = −0.00034
  （皆为负，Δ_new < Δ_dup）**——在固定优化预算（800 iters × restarts=1）
  下 N≥3 后边际信息被优化难度代价淹没；新光引入新 ReLU 结构更难优化。
  这是**估计器预算-可辨识性**发现，**不是信息量否证**；数据修复后
  该实验必须让预算随 N 缩放（restarts≥2 + iters↑）重做。

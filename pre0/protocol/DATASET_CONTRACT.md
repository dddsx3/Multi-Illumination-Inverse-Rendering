...
**本数据集的 5 张光照图是同一幅图像**（差异仅为路径追踪噪声，见
`pre0/oracle_renderer/ORACLE_AUDIT.md §7`）。生成端 BlenderProc 光照
逐帧动画未生效；sh_coeffs 的 5 行参数真实不同但图像未随之变化。
使用本数据集的一切多光照结论无效。修复数据前，本合同中
"5 张图 = 5 盏不同光照"的语义描述仅代表设计意图，不代表事实。

## 9. 已钉死的语义冲突清单（PRE-01 审计对象）

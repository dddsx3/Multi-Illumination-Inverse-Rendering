...
if [[ -n "$RESUME_CKPT" && -f "$RESUME_CKPT" ]]; then
  # n5rgb 臂本地已到 epoch 63：放到 epoch_0063 位置即可被 run_a10.py 识别续跑
...
cd "$REPO"
python3 run_a10.py --data_root "$DATA_DIR" --plan-only --budget-hours "${BUDGET_HOURS:-6}"

echo
echo "== [5/5] 正式开跑 =="
echo "确认上面的计划无误后执行（建议放 nohup/tmux 里，断连不影响）："
echo
echo "  cd $REPO && nohup python3 run_a10.py \\"
echo "      --data_root $DATA_DIR --budget-hours ${BUDGET_HOURS:-6} \\"
echo "      > _a10_main_log.txt 2>&1 &"
echo
echo "  # 看进度：  tail -f $REPO/_a10_main_log.txt"
echo "  #           python3 run_a10.py --status"
echo
if [[ "${AUTO_START:-0}" == "1" ]]; then
  echo "AUTO_START=1 → 直接开跑"
  nohup python3 run_a10.py --data_root "$DATA_DIR" \
        --budget-hours "${BUDGET_HOURS:-6}" > _a10_main_log.txt 2>&1 &
  echo "已后台启动，PID $!"
fi
